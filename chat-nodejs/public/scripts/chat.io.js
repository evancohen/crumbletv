(function($){
	...
	var NICK_MAX_LENGTH = 15,
		...
		//Greeting that should be set by the streamer or default
		GREETING = "Welcome to my channel. Enjoy!",

		// server information
		serverAddress = 'http://162.248.167.139',
	...
	function bindDOMEvents(){
		...
		$('#identification').submit(function(e){
	...
	function bindSocketEvents(){
		...
		socket.on('connect', function(){
			...
			//TODO: attach an authentication token to this request
			//or just log in as a guest

			socket.emit('connect', {nickname: nickname, room: currentRoom});
		});
		...
		socket.on('roomclients', function(data){
			...
			//data.room is the room name
			insertMessage(serverDisplayName, GREETING , true, false, true);
	...
	function createRoom(){
		var room = $('#addroom input').val().trim();
	...
	function handleNickname(){
		var nick = $('#identification input').val().trim();
		var room = $('#addroom input').val().trim();
		setCurrentRoom(room);
		if(nick && nick.length <= NICK_MAX_LENGTH){
			...
			alert('NO NO NO. That is too long!');
		}
	}
